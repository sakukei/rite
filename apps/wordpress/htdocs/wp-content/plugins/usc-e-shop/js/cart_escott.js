jQuery(document).ready( function($) {
	...
	var token_dialog = '<div id="escott-token-dialog">'
		...
		+'</div>'
	+'</div>';
	var hidden_fields = '<input type="hidden" id="acting" value="" />';
	$("#delivery-info").append(token_dialog+hidden_fields);
...
jQuery( function($) {
	cartEScott = {
		...
		openTokenDialog: function() {
			...
			$.ajax({
				...
				if( $.trim(data[0]) == "OK" ) {
					...
					$("#escott-token-form").append(data[1]);
					if( $.trim(data[2]) == "quick" ) {
		...
		getToken: function() {
			...
			if( !check ) {
				//$("#escott-token-response").html(uscesL10n.escott_token_error_message);
