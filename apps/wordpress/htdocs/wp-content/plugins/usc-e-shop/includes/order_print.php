...
function usces_pdf_out($pdf, $data){
	...
	for ( $index = 0; $index < count($cart); $index++ ) {
		...
		$pdf_args = compact( 'page', 'x', 'y', 'onep', 'next_y', 'line_x', 'border', 'index', 'cart_row', 'fontsize', 'lineheight', 'linetop', 'font' );
...
function usces_pdfSetHeader($pdf, $data, $page, $font) {
	...
	$fontsizes = array( 'numbering_label'=>9, 'title'=>15, 'date'=>9, 'page_no'=>13, 'customer_company'=>12, 'customer_attn'=>8, 'customer_address'=>8, 
	...
	switch ( $_REQUEST['type'] ){
		...
		case 'nohin':
			...
			$juchubi = __('date of your order', 'usces').' : '.date(__('M j, Y', 'usces'), strtotime($data->order['date']));
	...
	$numbering_label = apply_filters('usces_filter_pdf_numbering_label', 'No.', $_REQUEST['type'], $data);
...
function usces_pdfSetFooter($pdf, $data, $font, $endflag) {
	...
	$pdf->MultiCell(37.7, $lineheight, usces_conv_euc(apply_filters('usces_filter_disnount_label', __('Campaign discount', 'usces'), $data )), $border, 'C');
		...
		if( usces_is_member_system() && usces_is_member_system_point() ) {
			if( 1 == usces_point_coverage() ) {
				if( 'products' == usces_get_tax_target() ){
					...
					$data_3 = apply_filters('usces_filter_cod_label', __('COD fee', 'usces'), $data->order['ID']);
				}else{
					$data_1 = apply_filters('usces_filter_shipping_label', __('Shipping', 'usces'));
					$data_2 = apply_filters('usces_filter_cod_label', __('COD fee', 'usces'), $data->order['ID']);
				...
				if( 'products' == usces_get_tax_target() ){
					...
					$data_4 = apply_filters('usces_filter_cod_label', __('COD fee', 'usces'), $data->order['ID']);
				}else{
	...
	list($fontsize, $lineheight, $linetop) = usces_set_font_size($fontsizes['footer_value']);
	$pdf->SetFont($font, '', $fontsize);
	// Footer value
	...
	if( 'end' == $endflag ){
		...
		$pdf->MultiCell(22.67, $lineheight, usces_conv_euc(apply_filters('usces_filter_total_full_price_value', $usces->get_currency($data->order['total_full_price']))), $border, 'R');
	}
...
function usces_get_pdf_myaddress($pdf, $lineheight){
	...
	case 'JP': 
		...
		if( !empty($usces->options['zip_code']) ){
