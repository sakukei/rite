	...
	function download_orderdetail_list() {
		...
		foreach((array)$rows as $data) {
			...
			for($i = 0; $i < count($cart); $i++) {
				...
				if(isset($_REQUEST['check']['tax'])) $line .= $td_h.usces_crform($data['tax'], false, false, 'return', false).$td_f;
...
//			$data = $wpdb->get_row( $query, ARRAY_A );
			...
			if(isset($_REQUEST['check']['note'])) $line .= $td_h.usces_entity_decode($note, $ext).$td_f;
			if(!empty($csod_meta)) {
				foreach($csod_meta as $key => $entry) {
					$name = $entry['name'];
					$csod_key = 'csod_'.$key;
