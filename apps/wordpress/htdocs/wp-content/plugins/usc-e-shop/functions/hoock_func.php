...
//				usces_save_order_acting_error( $log );
		...
		$_GET['uscesid'] = $datas['sesid'];
		if( empty($datas['sesid']) ){
			$log = array( 'acting'=>'zeus', 'key'=>$_REQUEST['sendpoint'], 'result'=>'SESSION ERROR', 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
...
//		$res = $usces->order_processing();
	...
	//*** zeus_card ***//
		...
		$rand = $_GET['sendpoint'];
		if( empty($rand) ){
			$log = array( 'acting'=>$_REQUEST['acting'], 'key'=>'(empty key)', 'result'=>$_REQUEST['result'], 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
		...
		if( 'OK' == $_REQUEST['result'] ){
			...
			$log = array( 'acting'=>$_REQUEST['acting'], 'key'=>$_REQUEST['sendpoint'], 'result'=>$_REQUEST['result'], 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
			header("HTTP/1.0 200 OK");
			die('error3');
		}
	...
	//*** zeus_bank ***//
		...
		if( '04' === $_REQUEST['status'] or '05' === $_REQUEST['status'] ) {
			$log = array( 'acting'=>$_REQUEST['acting'], 'key'=>$_REQUEST['sendpoint'], 'result'=>$_REQUEST['status'], 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
		...
		$table_name = $wpdb->prefix . "usces_order";
		$table_meta_name = $wpdb->prefix . "usces_order_meta";
		$mquery = $wpdb->prepare("SELECT order_id FROM $table_meta_name WHERE meta_key = %s", 'acting_'.$_REQUEST['tracking_no']);
		$order_id = $wpdb->get_var( $mquery );
		...
		if( $order_id == NULL ){
			...
			$res = $usces->order_processing();
			if( 'error' == $res ){
				$log = array( 'acting'=>$_REQUEST['acting'], 'key'=>$_REQUEST['sendpoint'], 'result'=>'ORDER DATA REGISTERED ERROR', 'data'=>$_REQUEST );
				usces_save_order_acting_error( $log );
				...
				$usces->set_order_meta_value( 'acting_'.$_REQUEST['tracking_no'], serialize($value), $order_id );
				$usces->set_order_meta_value( 'wc_trans_id', $_REQUEST['order_no'], $order_id );
		...
		if( false === $res ){
			...
			$log = array( 'acting'=>$_REQUEST['acting'], 'key'=>$_REQUEST['sendpoint'], 'result'=>'ORDER DATA UPDATE ERROR', 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
			usces_log('zeus bank error2 : update usces_order', 'acting_transaction.log');
			die('error2');
		}
		//usces_action_acting_getpoint( $order_id, $add_point );
		if( '03' === $_REQUEST['status'] ) usces_action_acting_getpoint( $order_id );
	...
	//usces_log('mquery2 : '.print_r($mquery, true), 'acting_transaction.log');
		...
		if(!$res){
			...
			$log = array( 'acting'=>$_REQUEST['acting'], 'key'=>$_REQUEST['sendpoint'], 'result'=>'ORDER META DATA UPDATE ERROR', 'data'=>$_REQUEST );
			usces_save_order_acting_error( $log );
	...
	//*** zeus_conv ***//
		...
		$table_name = $wpdb->prefix . "usces_order";
		$table_meta_name = $wpdb->prefix . "usces_order_meta";
		$mquery = $wpdb->prepare("SELECT order_id FROM $table_meta_name WHERE meta_key = %s", 'acting_'.$_REQUEST['sendpoint']);
		$order_id = $wpdb->get_var( $mquery );
		if( $order_id == NULL ){
...
/*			$res = $usces->order_processing();
			...
			if( '05' !== $_REQUEST['status'] ) {
				if( '04' === $_REQUEST['status'] ) {
					$res = usces_change_order_receipt( $order_id, 'receipted' );
				} else {
					$res = usces_change_order_receipt( $order_id, 'noreceipt' );
				}
				...
				//usces_action_acting_getpoint( $order_id, $add_point );
				if( '04' === $_REQUEST['status'] ) usces_action_acting_getpoint( $order_id );
			}
		}
